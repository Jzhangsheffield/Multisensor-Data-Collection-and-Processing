...
#     parser.add_argument(
#         '-o', '--output-dir', default=r"E:\multi_sensor_sync_save_data_test\evk4_test\Mike_run", help="Directory where to create RAW file with recorded event data")
#     args = parser.parse_args()
#     return args

root_dir = "E:\multi_sensor_sync_save_data_test\evk4_test"
task = "sample_prepare"
session = "Aug_08"
ambiguity = "clean"
participant = "M"
run = "run_1"
output_dir = os.path.join(root_dir, (task + '_' + session + '_' + ambiguity), participant, run)
...
def main():
    ...
    if device.get_i_events_stream():
        log_path = "recording_" + datetime.now().strftime("%y%m%d_%H%M%S_%f")  + ".raw"
        if output_dir != "":
