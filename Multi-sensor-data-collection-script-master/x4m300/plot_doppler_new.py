...
def prepare_for_plot(df):
    range_grouped = df.groupby(["FrameCounter", "RangeIdx"])["Doppler Power"].apply(
...
# 加载数据
df = pd.read_csv(r"F:/X4M300_radar/data/xethru_recording_20250502_095651_1a8dd80c-dc67-4b7f-bb9b-51bc2893fedf/doppler.csv")
df["Doppler Power"] = df["Doppler Power"].apply(ast.literal_eval)

# 分组
grouped = list(df.groupby("FrequencyStep"))
n_steps = len(grouped)
