...
def camera_thread(serial_number, save_dirs, min_dist, max_dist, param_save_dir):
...
def main():
    ...
    threads = []
    for serial in serial_numbers:
        t = threading.Thread(target=camera_thread, args=(serial, camera_dirs[serial], 0.0, 5.0, base_save_dir))
        t.start()
        threads.append(t)
