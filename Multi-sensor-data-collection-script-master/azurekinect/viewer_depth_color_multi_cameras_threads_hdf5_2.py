...
def camera_process(device_id: int, config: Config, hdf5_path: str, stop_event: multiprocessing.Event, start_event: multiprocessing.Event):
    ...
    with h5py.File(hdf5_path, "w") as h5f:
        ...
        append_frame(h5f, first_depth, first_rgb, time.time())
        frame_count = 1
...
def main():
    base_dir = r"C:\multi_sensor_sync_save_data_test\kinect"  # Adjust as needed
    task = 'sample_prepare'
    session = 'Aug_08'
    ambiguity = 'clean'
    participant = 'M'
    ...
    base_dir = os.path.join(base_dir, (task + '_' + session + '_' + ambiguity), participant, run)
