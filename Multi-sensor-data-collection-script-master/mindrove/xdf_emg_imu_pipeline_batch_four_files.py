...
# ============== 保存 CSV（含 board_ts） ==============
def save_emg_csv(y, env, board_ts, out_csv: Path):
    board_ts = board_ts[:y.shape[0]]
    head = ["board_ts"] + [f"emg{i+1}" for i in range(y.shape[1])]
    data = np.column_stack([board_ts, y])
    np.savetxt(out_csv, data, delimiter=",", header=",".join(head), comments="", fmt="%.7f")
    if env is not None:
        env_csv = out_csv.with_name(out_csv.stem.replace("_emg", "_emg_envelope") + out_csv.suffix)
        data2 = np.column_stack([board_ts[:env.shape[0]], env])
        head2 = ["board_ts"] + [f"env{i+1}" for i in range(env.shape[1])]
        np.savetxt(env_csv, data2, delimiter=",", header=",".join(head2), comments="", fmt="%.7f")

def save_imu_csv(acc, gyr, board_ts, out_csv: Path):
    board_ts = board_ts[:acc.shape[0]]
    head = ["board_ts"] + [f"acc_{ax}" for ax in "xyz"] + [f"gyro_{ax}" for ax in "xyz"]
    data = np.column_stack([board_ts, acc, gyr])
    np.savetxt(out_csv, data, delimiter=",", header=",".join(head), comments="", fmt="%.7f")
...
def required_image_paths(out_dir: Path, xdf_stem: str, sides, modality: str):
    ...
    for s in sides:
        if need_emg:
            req.append(out_dir / f"{xdf_stem}_{s}_emg.png")
        if need_imu:
            req.append(out_dir / f"{xdf_stem}_{s}_imu.png")
    return req
    ...
    try:
        ...
        if out_dir.exists() and all_exist(req_imgs):
            for side, stream in (("left", left_stream), ("right", right_stream)):
                ...
                rec = {"orig_n": orig_n}
                emg_csv = out_dir / f"{xdf_path.stem}_{side}_emg.csv"
                imu_csv = out_dir / f"{xdf_path.stem}_{side}_imu.csv"
                # 无论当前选择的模态，都尽量探测已有 CSV 行数（方便补写）
                rec["emg_csv_path"] = emg_csv if emg_csv.exists() else None
                rec["emg_csv_n"]    = count_csv_rows(emg_csv)
                rec["imu_csv_path"] = imu_csv if imu_csv.exists() else None
                rec["imu_csv_n"]    = count_csv_rows(imu_csv)
                summary[side] = rec
        ...
        def handle_stream(stream, side_label):
            ...
            if args.modality in ("emg", "both"):
                ...
                # CSV
                emg_csv_path = out_dir / f"{xdf_path.stem}_{side_label}_emg.csv"
                save_emg_csv(y, env, board_ts, emg_csv_path)
                rec["emg_csv_path"] = emg_csv_path
                rec["emg_csv_n"] = y.shape[0]
                # PNG
                emg_png = out_dir / f"{xdf_path.stem}_{side_label}_emg.png"
                try:
                    save_emg_plot(y, env, fs_emg, emg_png)
                except Exception as e:
                    print(f"[WARN] {xdf_path.stem}/{side_label} EMG 绘图失败：{e}")
            else:
                # 若未处理，尝试统计既有CSV（兼容之前已跑过的情况）
                emg_csv_path = out_dir / f"{xdf_path.stem}_{side_label}_emg.csv"
                rec["emg_csv_path"] = emg_csv_path if emg_csv_path.exists() else None
                rec["emg_csv_n"] = count_csv_rows(emg_csv_path)
            ...
            if args.modality in ("imu", "both"):
                ...
                # CSV
                imu_csv_path = out_dir / f"{xdf_path.stem}_{side_label}_imu.csv"
                save_imu_csv(acc_f, gyr_f, board_ts, imu_csv_path)
                rec["imu_csv_path"] = imu_csv_path
                rec["imu_csv_n"] = acc_f.shape[0]
                # PNG
                imu_png = out_dir / f"{xdf_path.stem}_{side_label}_imu.png"
                try:
                ...
                imu_csv_path = out_dir / f"{xdf_path.stem}_{side_label}_imu.csv"
                rec["imu_csv_path"] = imu_csv_path if imu_csv_path.exists() else None
                rec["imu_csv_n"] = count_csv_rows(imu_csv_path)
...
# ============== 主程序：批处理多个 XDF ==============
def main():
    pa = argparse.ArgumentParser("Batch XDF (two MindRove streams) -> per-file folder with left/right EMG+IMU CSV + PNG + summary.txt")
    ...
    pa.add_argument("--notch", nargs="*", type=float, default=50, help="如 --notch 50 100 150")
