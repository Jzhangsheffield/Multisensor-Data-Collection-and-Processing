...
#     def is_reviewed(k: Tuple[str, str, str]) -> bool:
#         rec = results.get(k)
#         if not rec:
#             return False
#         passfail = str(rec.get("是否通过", "")).strip()
#         clarity  = str(rec.get("清晰度", "")).strip()
#         return (passfail != "") and (clarity != "")

#     # 让程序从“第一个未 review 的片段”开始
#     i = 0
#     while i < len(items):
#         k = (items[i].letter, items[i].action, items[i].clip)
#         if not is_reviewed(k):
#             break
#         i += 1

#     if i >= len(items):
#         print(f"All clips already reviewed in CSV: {out_csv}")
#         return
...
#     while 0 <= i < total:
#     # 保险：如果当前段已 review，直接跳过
#         while i < total and is_reviewed((items[i].letter, items[i].action, items[i].clip)):
#             i += 1
#             if i >= total:
#                 break
...
#                         tmp_reason = ""
#                         tmp_follow = ""
#                         # 跳到下一段（并自动跳过已 review 的段）
#                         i += 1
#                         while i < total:
#                             nk = (items[i].letter, items[i].action, items[i].clip)
#                             if not is_reviewed(nk):
#                                 break
#                             i += 1
#                         break
...
def natural_sort_key(name: str):
    # 你的帧名是 20251113_170325_880571.jpg 这种，字典序基本就是时间序
    return name.lower()
...
def read_existing_csv(csv_path: Path) -> Dict[Tuple[str, str, str], Dict[str, str]]:
    """
    读取已有 review CSV。
    兼容旧 CSV：即使里面有额外列（例如你之前的“清晰度”），也会被忽略。
    """
    results: Dict[Tuple[str, str, str], Dict[str, str]] = {}
    if not csv_path.exists():
        return results
    ...
    for _, r in df.iterrows():
        key = (str(r["字母文件夹"]), str(r["动作文件夹"]), str(r["片段文件夹"]))
        results[key] = {
            "是否通过": str(r.get("是否通过", "")),
            "不通过理由": str(r.get("不通过理由", "")),
            "后续处理方式": str(r.get("后续处理方式", "")),
        }
    return results
...
def write_csv(csv_path: Path, items: List[ClipItem],
    ...
    for it in items:
        key = (it.letter, it.action, it.clip)
        rec = results.get(key, {"是否通过": "", "不通过理由": "", "后续处理方式": ""})
        rows.append({
            "字母文件夹": it.letter,
            "动作文件夹": it.action,
            "片段文件夹": it.clip,
...
def overlay_info(frame, it: ClipItem, idx: int, total: int,
    ...
    play_state = "PAUSED" if paused else "PLAYING"
    help_line = (
        "Keys: Enter=PASS | Down=FAIL->reason/follow | Right=Next | Left=Prev | "
        "Space=Pause/Play | A/D=Prev/Next frame | ESC=Save&Exit"
    )
...
def key_to_char(k: int) -> Optional[str]:
    """
    将 waitKeyEx 的返回值转换为可输入字符（输入模式用）。
    """
    if k < 0:
        return None
    if k in (KEY_LEFT, KEY_RIGHT, KEY_UP, KEY_DOWN):
...
def main():
    ...
    # 输入状态机：reason -> follow（仅 FAIL 需要）
    input_mode = False
    input_stage: Optional[str] = None  # None / "reason" / "follow"
    input_buffer = ""
    ...
    while 0 <= i < total:
        # 保险：如果当前段已 review，直接跳过（避免你 Left/Right 跳回来卡住）
        while i < total and is_reviewed((items[i].letter, items[i].action, items[i].clip)):
            i += 1
        if i >= total:
            break
